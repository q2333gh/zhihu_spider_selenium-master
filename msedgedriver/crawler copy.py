...
def login(driver):
    ...
    WebDriverWait(driver, timeout=60).until(
        lambda d: d.find_element(By.CLASS_NAME, "TopstoryTabs-link")
    )
    return driver
...
def crawl_article_detail(driver: webdriver):
    ...
    for website, title in website_col.items():
        ...
        temp_name = str(np.random.randint(999999999)) + str(
        ...
        try:
            WebDriverWait(driverkk, timeout=10).until(
                lambda d: d.find_element(By.CLASS_NAME, "ztext-math")
            )
        except:
            pass
        ...
        for i in range(len(pcontent)):
            ...
            while True:
                if imgpre >= len(imgheight):
                    break
                if i == 0 and imgheight[imgpre] < Pheight[i]:
                    imgtext += """<img src="%d.jpg" width="100%%"/>\n""" % number
                    imgpre += 1
                    number += 1
                elif (
                    i != len(pcontent) - 1
                    and imgheight[imgpre] > Pheight[i]
                    and imgheight[imgpre] < Pheight[i + 1]
                ):
                    imgtext += """<img src="%d.jpg" width="100%%"/>\n""" % number
                    imgpre += 1
                    number += 1
                elif i == len(pcontent) - 1 and imgheight[imgpre] > Pheight[i]:
                    imgtext += """<img src="%d.jpg" width="100%%"/>\n""" % number
                    imgpre += 1
                    number += 1
                else:
        ...
        if len(textlink) + len(rt) > 0:
            with open(
                os.path.join(dircrea, nam + "_formula_" + ".md"), "w", encoding="utf-8"
            ) as obj:
                obj.write("# " + tle + "\n\n")
                if len(rt) > 0:
                    obj.write(rt + "\n\n\n")
        ...
        if len(textlink) + len(rt) > 0:
            with open(
                os.path.join(dircrea, nam + ".txt"), "w", encoding="utf-8"
            ) as obj:
                if len(rt) > 0:
                    obj.write(rt + "\n\n\n")
        ...
        try:
            os.rename(
                os.path.join(dircrea, temp_name + ".pdf"),
                os.path.join(dircrea, nam + ".pdf"),
            )
        except:
...
def crawl_answer_detail(driver: webdriver):
    ...
    for website, title in website_col.items():
        ...
        richtext = QuestionAnswer.find_element(
            By.CLASS_NAME, "RichContent--unescapable"
        )
